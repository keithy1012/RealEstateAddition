...
def run(search_term):
    ...
    search_url = generate_url(search_term) #search_url variable is the generated URL link
    driver.get(search_url) #start running the webdriver 
    time.sleep(5) #sleeping to ensure all data is loaded before continuing
...
def GetSlope():
    return float("{:.2f}".format(float(line.coef_[0])))

def GetIntercept():
    return float("{:.2f}".format(float(line.intercept_[0])))
def GetScore():
    return Score
    
def GetPrice():
    #sqft = document.request.get('sqft')
    intercept = float(line.intercept_[0])
    slope = float(line.coef_[0])  
    #do equation
    print("Formula: Sqft = " , line.coef_[0] , '* Price + ' , line.intercept_[0])
#----------------------------------------------------
def RunEverything(Zip_Code):
    run(Zip_Code)
    getCSV(filename)
    PlotPoints(data)
